...
//aser una funcion CalculateAptitude que calcule la distancia total que de parametros sea vector de vectores

float CalculateAptitudePoblacion(vector<vector<Point>> Pobla)
{
	float anw = 0.0;
	for (size_t i = 0; i < Pobla.size(); i++)
	{
		anw = anw + CalculateAptitude(Pobla[i]);
	}
	return anw;
}
...
vector<vector<Point>> TournamentSelectionAndCrossing(vector<vector<Point>> poblationX)
{
	...
	{
		//llamar a calcular aptitud y comparar para sacar al mayor y este guardar en el resultado
